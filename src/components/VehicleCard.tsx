...
export function VehicleCard({ data }: VehicleCardProps) {
  ...
  return (
    <div className="grid grid-cols-12 gap-4 md:gap-6 print:block">
      {/* HEADER SECTION: THE HERO STATUS */}
      <motion.div 
          ...
          <div className="space-y-6 md:space-y-8">
            <div className="flex items-center gap-6">
               ...
               <div className="flex items-center gap-2">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Interno: {data.interno}</span>
               </div>
            </div>
            
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-white tracking-[-0.04em] leading-[0.9]">
              {data.modelo?.split(' ')[0]} <span className="text-blue-500 italic">{data.modelo?.split(' ').slice(1, 3).join(' ')}</span>
              <span className="block text-slate-500 font-medium mt-2 text-xl md:text-3xl tracking-tight">{data.modelo?.split(' ').slice(3).join(' ')}</span>
            </h2>

            <div className="flex flex-wrap gap-8 md:gap-12 pt-2">
               <HeroStat label="Color Exterior" value={data.color || 'N/A'} icon={<Palette className="w-4 h-4 md:w-5 md:h-5" />} />
               <HeroStat label="Operación" value={data.tipoDeVenta} icon={<Zap className="w-4 h-4 md:w-5 md:h-5" />} />
               {shouldShowDate && (
                 <HeroStat label="Agenda Entrega" value={`${data.fecha} • ${data.hora || '--'} HS`} icon={<Calendar className="w-4 h-4 md:w-5 md:h-5" />} />
               )}
            </div>
          </div>
          ...
          <div className="w-full xl:w-[450px] space-y-8 md:space-y-10 bg-white/5 p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-white/5 backdrop-blur-sm">
            <div className="flex justify-between items-end">
               <div className="space-y-2">
                 <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Último Estado</span>
               ...
               <div className="text-right">
                 ...
                 <p className="text-3xl md:text-4xl font-black text-blue-500 tracking-tighter">{Math.round(getProgressPercentage())}%</p>
               </div>
            </div>
      ...
      <div className="col-span-12 xl:col-span-8 space-y-4 md:space-y-6">
        <div className="glass-panel p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem]">
          <div className="flex justify-between items-center mb-10 md:mb-14 px-4">
             <div>
                <h3 className="text-xl md:text-2xl font-black text-white tracking-tight">Timeline Estratégico</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
           <InfoPanel 
             ...
             value={data.vendedor || 'Punto de Venta Directo'}
             sub="Atención personalizada durante todo el proceso"
           />
           <InfoPanel 
             icon={<Truck className="w-6 h-6 md:w-7 md:h-7 text-emerald-500" />}
             title="Logística Comercial"
      ...
      <div className="col-span-12 xl:col-span-4 space-y-4 md:space-y-6">
        ...
        >
          ...
          <div className="relative z-10 space-y-8">
            ...
            <div className="space-y-6 md:space-y-8">
               <ContactItem icon={<Smartphone className="w-4 h-4 md:w-5 md:h-5 text-blue-500" />} label="Línea Directa" value={data.telefono || 'No disponible'} />
               <ContactItem icon={<Mail className="w-4 h-4 md:w-5 md:h-5 text-blue-500" />} label="Email Principal" value={data.mail || 'No disponible'} />
            </div>
          </div>
...
const TimelineStep: React.FC<TimelineStepProps> = ({ label, active, isCurrent, index }) => {
  return (
    <div className="flex flex-col items-center gap-4 md:gap-6 relative group">
       <div className={`w-12 h-12 md:w-16 md:h-16 rounded-[1.2rem] md:rounded-[1.5rem] flex items-center justify-center transition-all duration-700 relative z-10 border-2 md:border-4 ${
         active 
       ...
       <div className="text-center space-y-1">
          ...
          {isCurrent && <span className="text-[8px] font-black text-blue-500 uppercase tracking-tighter">Gestión Activa</span>}
       </div>
    </div>
  );
}
